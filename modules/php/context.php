    ...
    function isActionPossible($situation) {
        ...
        switch ($action) {
            ...
            case ACTION_SWAP_MY_TICKET:
                ...
                return count($this->getTicketsFromPlayerOnFestival($playerId, $festivalId)) > 0 && $this->array_some(array_keys($ticketsByFest), fn ($festId) => $festId != $festivalId && $this->array_some($ticketsByFest[$festId], fn ($t) => $t->type_arg != $this->getColorFromHexValue($color)));
                break;
